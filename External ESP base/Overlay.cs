    ...
    public partial class Overlay : Form
    {
        ...
        private void Update(object sender)
        {
            while (IsGameRun())
            {
                ...
                if (!IsMinimized)
                {
                    ...
                    // Draw Credits
                    DrawTextCenter(rect.Width / 2 - 125, 5, 250, (int)font.FontSize, "EXTERNAL ESP BASE BY XTREME2010", new Color(255, 214, 0, 255), true);
        ...
        private void Read()
        {
            ...
            for (uint i = 0; i < 70; i++)
            {
                ...
                // Name
                player.Name = RPM.ReadString(pEnemyPlayer + Offsets.ClientPlayer.szName, 10);
                ...
                if (player.IsValid())
                {
                        ...
                        WorldToScreen(player.Origin, player.Pose, out w2sHead))
                    {
                        ...
                        if (ESP_Distance)
                        {
                            DrawText((int)X, (int)w2sFoot.Y, (int)player.Distance + "m", Color.White, true);
                        }
        ...
        private void DrawMenu(int X, int Y)
        {
            ...
            DrawText(X + 430, Y, "F9: ESP Vehicle", ESP_Vehicle ? selectedColor : Color.White, true, fontSmall);
        }
